...
4. **Register the 604 page fonts only after they pass their hash.** Fonts arrive in the downloaded, verified core pack — not the app bundle — so load them at runtime with `FontLoader(qpcFontFamily(page))..addFont(...)` over bytes read through `vault.readVerified(expectedSha256: ed.fontSha256[page])`, which throws on mismatch. Refuse to register an unverified font. `docs/engineering/08-quran-data-and-immutable-rendering.md` §2 (`registerVerifiedPageFonts`, `FontLoader`); `docs/PRD.md` §11.1.1 (refuse to render unverified assets).

...
**In practice.**
- Use `Switch.adaptive` for settings toggles, `CircularProgressIndicator.adaptive` for an optional reciter-pack download (PRD §11.1; the core is bundled, no download), `AlertDialog.adaptive` for confirmations (erase, import/replace), `Slider.adaptive` for the Quran zoom/font-size control — each renders the Cupertino control on iOS while keeping our seeded `ColorScheme` colors, so the app reads as native iOS without a green-system-default look.

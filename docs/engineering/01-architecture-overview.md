...
**Decision.** The core muṣḥaf is **bundled in the signed binary** (build-time-verified), so the network is never in the critical path; the *only* permitted network client is the **optional-pack downloader** (reciter audio, alt-muṣḥaf — HTTPS GET to GitHub Releases/CDN, carrying no auth, cookies, or identifiers). "Fully offline" is a **build invariant**, not a promise, enforced by a two-layer CI gate plus test-time network blocking (Decision log: *no networking beyond asset download* (amended); [PRD C1, §17, §19.3, §20 gate 6](../PRD.md)).
...
**1. The network is quarantined to one module.** Only `assets/` may open a socket, and only for **optional** packs; it loads raw bytes, holds no state, and is wrapped by an asset repository that verifies SHA-256 against the binary's **pinned manifest** before anything renders — fail-closed: any mismatch ⇒ reject, re-fetch once, then refuse to render Quran text. The **bundled core** is verified against the same manifest at build time and re-checked at first load ([09-asset-packs-and-offline-integrity.md](09-asset-packs-and-offline-integrity.md); Decision log: *Quran asset distribution & offline integrity* (amended)). Every user-data repository in `data/` imports no networking package at all, so the "no per-user data leaves the device" claim is provable by reading the `data/` import lines.

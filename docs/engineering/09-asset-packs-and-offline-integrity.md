...
This document specifies how Hifz Companion gets the Quran onto the device and *proves* the bytes are exactly the authoritative muṣḥaf — once, over the network, and then never again. It covers where the asset packs are hosted (GitHub immutable Releases, pinned by exact tag), how the one-time download works (the single network event in the product's life), how every downloaded file is re-verified at runtime against a SHA-256 manifest baked into the signed app binary (fail-closed — a mismatch refuses to render Quran text), how the packs are built reproducibly so the community can re-derive our hashes, and how "offline forever" is enforced as a build invariant rather than a promise. It applies the *Decision log: Quran asset distribution & offline integrity* entry (README decision 5) and the *Decision log: No networking beyond asset download* entry (README decision 8), and is grounded in the evidence dossier [research/asset-integrity-and-distribution.md](research/asset-integrity-and-distribution.md) and [PRD §11.1, §11.1.1, §17, §19.3, §20](../PRD.md).

The boundaries are deliberate. This doc owns the *acquisition and integrity* of the asset packs — the downloader, the manifest, the verifier, the local cache, and the offline guarantee. What the *verified* assets then become — the immutable glyph-font rendering, the overlay painter, the layout geometry — is owned by [08-quran-data-and-immutable-rendering.md](08-quran-data-and-immutable-rendering.md). The live Drift store the reference DB is built into is owned by [05-persistence-and-encryption.md](05-persistence-and-encryption.md). The CI gates that prove no second network client exists are shared with [11-testing-strategy.md](11-testing-strategy.md), and the license/attestation/release-channel mechanics that publish the packs are shared with [13-oss-repo-and-release.md](13-oss-repo-and-release.md). This doc owns the trust boundary between "untrusted network" and "verified local muṣḥaf."

Two framing rules from the README's outranking rules and [PRD R1, C1](../PRD.md) govern everything below. First: **the sacred text is never rendered from an unverified byte.** A single altered diacritic ends the project, so the integrity gate is fail-closed — when in doubt, refuse, never degrade. Second: **the one-time download is the only moment the app extends trust to the network, and after it the app phones home to nothing, ever.** The verified local muṣḥaf is the root of trust for the lifetime of the install; there is no remote config, no silent re-fetch, no "latest" check, and no first-party server to receive anything.

## At a glance

| Concern | Decision |
|---|---|
| Pack hosting | **GitHub immutable Releases** (GA 28 Oct 2025), referenced by **exact tag** baked into the binary — never `latest` ([GitHub: Immutable releases](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases)) |
...
| Download client | One HTTPS GET, no auth/cookies/identifiers; `dio` with `CancelToken` + timeouts ([pub.dev: dio](https://pub.dev/packages/dio)) |
...
| Offline guarantee | A **build invariant**: CI bans all networking outside the one downloader module; airplane-mode acceptance test ([F-Droid: Inclusion Policy](https://f-droid.org/en/docs/Inclusion_Policy/)) |
...
Quran assets ship as **versioned offline packs** — a required **core** pack plus optional **reciter-audio** and future **alt-muṣḥaf** packs — published to **GitHub immutable Releases** on the public, open-source data repository, and referenced by an **exact release tag baked into the app binary, never `latest`** (*Decision log: Quran asset distribution & offline integrity*). The app treats a 404 on a pinned asset URL as "keep using the verified local copy," never as a trigger to fetch something else. No first-party server, API, or CDN we operate exists anywhere in this picture.
...
| Pack | Contents | When fetched | Source / license |
|---|---|---|---|
| **Core (required)** | Uthmani text (Tanzil, verbatim + attribution); QUL page/line layout; 604 KFGQPC QPC glyph fonts; scholar-reviewed mutashābihāt dataset | Once, at onboarding — the single moment network is needed | Tanzil (verbatim copy + link); QUL/KFGQPC (per-resource terms) ([Tanzil: Terms](https://tanzil.net/download/); [QUL: QPC font](https://qul.tarteel.ai/resources/font/249)) |
...
A versioned **pack manifest** file (itself an asset in the release, *and* its expected hash baked into the binary) declares every file in the pack:
...
The core pack is downloaded **once, during onboarding**, by a **single whitelisted downloader module** in `/assets` — a plain HTTPS GET that carries **no authentication, cookies, or identifiers**, only the public asset URL. Downloads go to the **temporary directory**, are verified there (§3), and only a verified pack is moved into app-documents. The download is cancellable, resumable on retry, and — if the device is offline at first run — surfaces a calm, honest "one-time download needed" state with a retry, after which the app is permanently offline (*Decision log: No networking beyond asset download*; [PRD §11.1.1, §12.1](../PRD.md)).
...
- **Download-to-temp-then-promote prevents rendering from an in-flight file.** The convention is durable, hard-to-re-download data in documents and cheap re-downloadable in-flight data in temp ([DhiWise: app documents dir](https://www.dhiwise.com/post/a-deep-dive-into-getapplicationdocumentsdirectory)). The verified core pack is the irreplaceable muṣḥaf, so it lives in documents; an unverified download is temp and is never read as Quran (research note, Finding 9).
...
The onboarding flow ([PRD §12.1](../PRD.md)) sequences download → verify → build-DB so that no partially-trusted state is ever observable:

```dart
Future<CorePackResult> installCorePack({required CancelToken cancel}) async {
  final manifest = EmbeddedManifest.core;        // baked into the binary
  final verifiedFiles = <String, File>{};

  for (final entry in manifest.files) {          // text, layout, mutashābihāt, 604 fonts
    final tmp = await _downloader.fetchToTemp(entry.name, cancel: cancel);
    final actual = await sha256OfFile(tmp);       // §3 — chunked hash
    if (actual != entry.sha256) {
      await tmp.delete();
      final retry = await _downloader.fetchToTemp(entry.name, cancel: cancel);
      if (await sha256OfFile(retry) != entry.sha256) {
        await retry.delete();
        return CorePackResult.integrityFailure(entry.name);   // FAIL-CLOSED (§3)
      }
      verifiedFiles[entry.name] = retry;
    } else {
      verifiedFiles[entry.name] = tmp;
    }
  }
  // Only now, with EVERY file verified, promote into app-documents and build the DB.
  await _promoteToDocuments(verifiedFiles);
  await _buildReferenceDb(verifiedFiles);         // → /data reference tables (05)
  await _appMeta.set('text_checksum_verified_at', DateTime.now().toUtc());
  return CorePackResult.ready();
}
```

If the device is offline at first run, the flow yields a distinct, honest state — not an error that blames the user:

| Condition | App state | Copy posture |
|---|---|---|
| Offline at first run | `awaitingFirstDownload` | "Hifz Companion needs a one-time download to set up your muṣḥaf. You can do this on any connection, once." + Retry |
| Download interrupted | `downloadInterrupted` | Resume/retry; the partial `.part` file is discarded, never read |
| All files verified | `ready` | The single network moment is over; the app is offline forever |
...
- **We refuse to treat "offline at first run" as a failure to scold.** The honest message is that one download is needed once; the tone is calm and non-blaming ([PRD §12.1](../PRD.md), R3).
...
The one-time download enforces **modern TLS (1.2+/1.3)** and relies on the platform trust store, but **does not pin GitHub's TLS certificate or public key**. End-to-end integrity is carried by the **content hash** (§3), not the transport (*Decision log: Quran asset distribution & offline integrity*; research note, Finding 10).
...
"Fully offline after setup" is enforced as a **build invariant and a release gate**, not asserted in prose. A **two-layer CI gate** proves the only network client is the asset downloader: (a) a dependency allow-list step fails on any analytics/ads/backend/crash SDK in the resolved graph; (b) a banned-import rule forbids networking imports everywhere except the single `/assets` downloader module. A **manual airplane-mode acceptance test** confirms every Quran feature works indefinitely after the one-time download (*Decision log: No networking beyond asset download*; [PRD §19.3, §20.6](../PRD.md)).
...
| Airplane-mode acceptance | Manual + scripted journey | Install → core download → enable airplane mode → any Quran feature fails ([PRD §20.6](../PRD.md)) |
...
The airplane-mode journey is the human-facing proof of the whole chapter: after the single verified download, the muṣḥaf reader, the Today scheduler, grading, the heat-map, mutashābihāt drills, notifications, and backup all work with the radio off — because the verified local muṣḥaf is the root of trust and nothing else is ever fetched.
